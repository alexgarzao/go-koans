...
func aboutArrays() {
	...
	assert(cap(tasty_fruits) == 3) // but its capacity is surprising! [1]

	// [1] In really, slice always determine the tasty_fruits's capacity counting
	// from start until fruit's capacity.

	tasty_fruits[0] = "lemon" // are their shared roots truly identical? [2]
	// [2] tasty_fruits has a reference for fruits
